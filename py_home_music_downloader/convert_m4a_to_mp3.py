...
def get_all_file_paths(project_music_dir):
    ...
    for file in music_list:
        ...
        file_is_dir = pydpzpath.isdir(file_path)
        ...
        if not file_is_dir:
            if not file_is_mp3 and file_is_audio_file:
                ...
                cmd_string = "\"C:\\Program Files\\VideoLAN\VLC\\vlc.exe\" -I dummy \"" + str(file_path) + "\" \":sout=#transcode{acodec=mpga,ab=192}:std{dst=\'" + str(new_file_path) + "\',access=file}\" vlc://quit"
            ...
            for album in album_list:
                ...
                file_is_dir = pydpzpath.isdir(artist_dir_song_path)
                ...
                if not file_is_dir:
                    if not file_is_mp3 and file_is_audio_file:
                        ...
                        cmd_string = "\"C:\\Program Files\\VideoLAN\VLC\\vlc.exe\" -I dummy \"" + str(artist_dir_song_path) + "\" \":sout=#transcode{acodec=mpga,ab=192}:std{dst=\'" + str(new_artist_dir_song_path) + "\',access=file}\" vlc://quit"
                        cmd_string_list.append(cmd_string)
                else:
                    ...
                    for song in song_list:
                        ...
                        file_is_dir = pydpzpath.isdir(song_file_path)
                        ...
                        if not file_is_dir:
                            if not file_is_mp3 and file_is_audio_file:
                                ...
                                cmd_string = "\"C:\\Program Files\\VideoLAN\VLC\\vlc.exe\" -I dummy \"" + str(song_file_path) + "\" \":sout=#transcode{acodec=mpga,ab=192}:std{dst=\'" + str(new_song_file_path) + "\',access=file}\" vlc://quit"
                                cmd_string_list.append(cmd_string)
                        else:
                            print("done")
